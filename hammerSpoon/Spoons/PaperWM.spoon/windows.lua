...
function Windows.addWindow(add_window)
	...
	if
		add_window:tabCount() > 1
		and add_window:application():bundleID():sub(1, #apple) == apple
		and add_window:application():bundleID():sub(1, #safari) ~= safari
	then
	...
	if Windows.PaperWM.state.windowIndex(add_window) then
		return
	end
	...
	if
		Windows.PaperWM.state.prev_focused_window
		and ((Windows.PaperWM.state.windowIndex(Windows.PaperWM.state.prev_focused_window) or {}).space == space)
		and (Windows.PaperWM.state.prev_focused_window:id() ~= add_window:id())
...
function Windows.removeWindow(remove_window, skip_new_window_focus)
	...
	if not skip_new_window_focus then -- find nearby window to focus
		for _, direction in ipairs({
			...
			if Windows.focusWindow(direction, remove_index) then
				break
			end
		end
	end
	...
	assert(
		remove_window
			== table.remove(Windows.PaperWM.state.windowList(remove_index.space, remove_index.col), remove_index.row)
	)
...
function Windows.focusWindow(direction, focused_index)
	...
	if direction == Direction.LEFT or direction == Direction.RIGHT then
		...
		for row = focused_index.row, 1, -1 do
			new_focused_window =
				Windows.PaperWM.state.windowList(focused_index.space, focused_index.col + direction, row)
	...
	elseif direction == Direction.UP or direction == Direction.DOWN then
		new_focused_window = Windows.PaperWM.state.windowList(
			focused_index.space,
			focused_index.col,
	...
	elseif direction == Direction.NEXT or direction == Direction.PREVIOUS then
		...
		new_focused_window =
			Windows.PaperWM.state.windowList(focused_index.space, focused_index.col, focused_index.row + diff)
...
function Windows.cycleWindowSize(direction, cycle_direction)
	...
	local function findNewSize(area_size, frame_size, cycle_direction, dimension)
		...
		elseif cycle_direction == Direction.DESCENDING then
			...
			Windows.PaperWM.logger.e("cycle_direction must be either Direction.ASCENDING or Direction.DESCENDING")
		end
	...
	elseif direction == Direction.HEIGHT then
		...
		Windows.PaperWM.logger.e("direction must be either Direction.WIDTH or Direction.HEIGHT")
		return
	end
...
function Windows.splitScreen()
	...
	Windows.PaperWM.tiling.tileColumn(
		left_column,
		left_bounds,
		nil,
		half_width - Windows.PaperWM.windows.getGap("left")
	)
